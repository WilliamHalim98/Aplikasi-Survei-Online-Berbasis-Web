...
async function getQuestionnaireDetails(){
  let response = await fetch(`${url}/getQuestionnaireById`,{
  ...
  fillQuestionnaireDescription(item[0].questionnaire_title,item[0].questionnaire_description);
};
...
async function answerQuestions(){
	...
	let data = await fetch(`${url}/getQuestions`,{
    method: "POST", // *GET, POST, PUT, DELETE, etc.
    mode:"cors",
    headers: {
      'Content-Type': 'application/json',
   ...
   for(let i=0;i<result.length;i++){
   	let answersRadio =[];
	let answersCheckbox =[];
   	if(result[i].question_type === "text"){
   	...
   	if(result[i].question_type === "radio"){
   		const jawabanRadio = {
   			question_id:i,
   			answer: answersRadio,
   		}
   	...
   	if(result[i].question_type === "checkbox"){
   ...
   console.log(JSON.stringify(answerLists));
	let responses = await fetch(`${url}/answer`, {
    method: "POST", // *GET, POST, PUT, DELETE, etc.
    mode:"cors",
    headers: {
      'Content-Type': 'application/json',
    ...
    body: JSON.stringify({
      "questionnaire_id": localStorage.getItem('answererQuestionnaireId'),
      "respondent_name": localStorage.getItem('answererName'),
      "respondent_email": localStorage.getItem('answererEmail'),
      "respondent_company": localStorage.getItem('answererCompany'),
      "answers":answerLists,
    }),
  });
